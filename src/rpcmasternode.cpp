...
static CMasternode::Tier GetMasternodeTierFromString(std::string str)
{
    ...
    if(str == "bronze") {
        return CMasternode::MASTERNODE_TIER_BRONZE;
    }
    else if(str == "silver") {
...
Value allocatefunds(const Array& params, bool fHelp)
{
	if (fHelp || params.size() != 3)
		throw runtime_error(
			...
			"3. amount			(gold, silver, bronze) tier of masternode. \n"
			"      <future>     (numeric, required) amount of dmtc funded will also be accepted for partially funding master nodes and other purposes.\n"
...
Value fundmasternode(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 4)
		throw runtime_error(
			...
			"2. amount			(gold, silver, bronze) tier of masternode. \n"
			"      <future>     (numeric, required) amount of dmtc funded will also be accepted for partially funding master nodes and other purposes.\n"
...
{
	string ret = indent + "\"address\": \"address\",    (string) Masternode DMTC address\n";
