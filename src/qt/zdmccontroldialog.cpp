...
#include "ui_zdmccontroldialog.h"
...
std::list<std::string> ZDmcControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZDmcControlDialog::listMints;

ZDmcControlDialog::ZDmcControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZDmcControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZDmcControlDialog::~ZDmcControlDialog()
{
    delete ui;
}

void ZDmcControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZDmcControlDialog::updateList()
{
...
void ZDmcControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZDmcControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZDmc_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZDmcControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZDmcControlDialog::GetSelectedMints()
{
...
void ZDmcControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
