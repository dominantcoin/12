...
void SendCoinsDialog::send(QList<SendCoinsRecipient> recipients, QString strFee, QStringList formatted)
{
    ...
    // add total amount in all subdmtcsion units
...
void SendCoinsDialog::splitBlockLineEditChanged(const QString& text)
{
    ...
    //if greater than 0 then dmtcde after fee by the amount of blocks
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel()) {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid DMTC address"));
        } else // Valid address
        {
            CPubKey pubkey;
            CKeyID keyid;
