                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zDMC ought to be enough for anybody." - Bill Gates, 2017
    ui->zDMCpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzDMCSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzDMC->setEnabled(false);
        ui->pushButtonMintzDMC->setToolTip(tr("zDMC is currently disabled due to maintenance."));

        ui->pushButtonSpendzDMC->setEnabled(false);
        ui->pushButtonSpendzDMC->setToolTip(tr("zDMC is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zDMCpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzDMC_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zDMC is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zDMC...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zDMC in ") + 
...
void PrivacyDialog::on_pushButtonSpendzDMC_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zDMC is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zDMC
        sendzDMC();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zDMC
    sendzDMC();
}

void PrivacyDialog::on_pushButtonZDmcControl_clicked()
{
    ZDmcControlDialog* zDmcControl = new ZDmcControlDialog(this);
    zDmcControl->setModel(walletModel);
    zDmcControl->exec();
}

void PrivacyDialog::setZDmcControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzDmcSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzDMC()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Dmtc Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zDMCpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zDMC is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " DMC </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zDMC</b>";
    ...
    // use mints from zDmc selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZDmcControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZDmcControlDialog::GetSelectedMints();
    }
    ...
    if (!fSuccess) {
        ...
        const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zDMC transaction
    ...
    // Clear zdmc selector in case it was used
    ZDmcControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zDmc Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Dmc, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zDmc Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zDMC </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zDMC "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zDMC "));
    ui->labelzDMCAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzDMCSyncStatus->setVisible(fShow);
}
