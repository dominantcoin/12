...
Value GetNetworkHashPS(int lookup, int height)
{
    ...
    // In case there's a situation where minTime == maxTime, we don't want a dmtcde by zero exception.
    if (minTime == maxTime)
        return 0;
...
Value getgenerate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or dmtc.conf setting gen)\n"
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "DMTC is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "DMTC is downloading blocks...");
