...
const int TIER_BRONZE_BASE_COLLATERAL   = 800;
const int TIER_SILVER_BASE_COLLATERAL   = 2000;
const int TIER_GOLD_BASE_COLLATERAL     = 4000;

static size_t GetHashRoundsForTierMasternodes(CMasternode::Tier tier)
{
    switch(tier)
    {
    case CMasternode::MASTERNODE_TIER_BRONZE:   return 20;
...
CAmount CMasternode::GetTierCollateralAmount(CMasternode::Tier tier)
{
    switch(tier)
    {
    case MASTERNODE_TIER_BRONZE:   return TIER_BRONZE_BASE_COLLATERAL * COIN;
...
{
    if(TIER_BRONZE_BASE_COLLATERAL * COIN == nCollateral) {
        return MASTERNODE_TIER_BRONZE;
    }
...
bool CMasternode::IsTierValid(CMasternode::Tier tier)
{
    switch(tier)
    {
    case MASTERNODE_TIER_BRONZE:
    case MASTERNODE_TIER_SILVER:
    case MASTERNODE_TIER_GOLD: return true;
    case MASTERNODE_TIER_INVALID: break;
    }
...
std::string CMasternode::TierToString(CMasternode::Tier tier)
{
    switch(tier)
    {
    case MASTERNODE_TIER_BRONZE: return "BRONZE";
