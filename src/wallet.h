...
// Possible states for zDMC send
enum ZerocoinSpendStatus {
    ZDMC_SPEND_OKAY = 0,                            // No error
    ZDMC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZDMC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZDMC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZDMC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZDMC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZDMC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZDMC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZDMC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZDMC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZDMC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZDMC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZDMC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZDMC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZDMC_SPENT_USED_ZDMC = 14,                      // Coin has already been spend
    ZDMC_TX_TOO_LARGE = 15                          // The transaction is larger than the max tx size
};
...
public:
    ...
    */
    ...
    void setZDmcAutoBackups(bool fEnabled)
    {
