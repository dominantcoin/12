...
base_uint<BITS>& base_uint<BITS>::operator/=(const base_uint& b)
{
    ...
    *this = 0;                   // the quotient.
    ...
    if (div_bits == 0)
        throw uint_error("Dmtcsion by zero");
    ...
    // num now contains the remainder of the dmtcsion.
    return *this;
}
...
uint32_t uint256::GetCompact(bool fNegative) const
{
    ...
    // Thus, if it is already set, dmtcde the mantissa by 256 and increase the exponent.
