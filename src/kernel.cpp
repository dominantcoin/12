        ...
        const CBlockIndex** pindexSelected)
{
    ...
    *pindexSelected = (const CBlockIndex*)0;
    BOOST_FOREACH (const PAIRTYPE(int64_t, uint256) & item, vSortedByTimestamp) {
        ...
        // the selection hash is dmtcded by 2**32 so that proof-of-stake block
...
{
    //Dmtc will hash in the transaction hash and the index number in order to make sure each hash is unique
