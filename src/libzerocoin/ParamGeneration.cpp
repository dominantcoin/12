                    ...
                    uint32_t *prime_gen_counter)
{
	...
	if (primeBitLen < 33) {
		...
		while ((*prime_gen_counter) < (4 * primeBitLen)) {
			...
			// Perform trial dmtcsion on this (relatively small) integer to determine if "intc"
			// is prime. If so, return success.
			if (primalityTestByTrialDmtcsion(intc)) {
...
/// \brief Determines whether a uint32_t is a prime through trial dmtcsion.
/// \param candidate       Candidate to test.
/// \return                true if the value is prime, false otherwise
///
/// Performs trial dmtcsion to determine whether a uint32_t is prime.

bool
primalityTestByTrialDmtcsion(uint32_t candidate)
{
