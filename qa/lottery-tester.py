...
def main(argv):
    ...
    parser.add_argument('--prefix', dest='prefix', help='path to folder where dmtc-cli is located', default='.')
    ...
    executableName = prefix + '/dmtc-cli'
    print executableName

    if os.path.isfile(executableName) == False:
        print 'Cannot find dmtc-cli, consider using --prefix'
        sys.exit(0)

    try:
        json.loads(executeProcess([executableName, 'getinfo']))
    except ValueError:
        print 'Dmtc-cli is not running, or it is in wrong dir'
        sys.exit(0)
